...
{
   Ocrivist_Spell.pas is a slightly enhanced version of the unit Speller.pp
   which can be found in the fpc src/packages/aspell directory
}
...
type
  ...
  TWordSpeller = class(TSpeller) // class for simple per-word checking
   ...
   public
    ...
    procedure AddToPersonalDict( Word: string );
    procedure AddToSession( Word: string );
...
begin
  aspell_speller_add_to_session(FSpeller, pChar(Word), Length(Word));
end;

procedure TWordSpeller.SaveWordlists;
begin
