...
uses
  ...
  DelphiTwainUtils, Messages, ExtCtrls, Forms;

const
  {Name of the Twain library for 32 bits enviroment}
  TWAINLIBRARY: AnsiString = 'TWAIN_32.DLL';
...
type
  ...
  private
    {Owner}
    fOwner: {$IFNDEF DONTUSEVCL}TObject{$ELSE}TObject{$ENDIF};
  ...
  public
    ...
    {$IFNDEF DONTUSEVCL} constructor Create(AOwner: TObject);
  ...
  public
    ...
    property TransferMode: TTwainTransferMode read fTransferMode write fTransferMode; // <-- ONLY ttmNative works properly!!!
  public
...
{Units used bellow}
uses
  Controls, uFormSelectSource{$IFDEF FPC}, interfacebase{$ENDIF};

{Called by Delphi to register the component}
(*
//ONDREJ: DO NOT USE DelphiTwain IN DESIGN TIME !!!!
procedure Register();
begin
  {$IFNDEF DONTUSEVCL}
...
{Object being created}
{$IFNDEF DONTUSEVCL} constructor TTwainIdentity.Create(AOwner: TObject);
...
begin
  ...
  begin
    ...
    TwainDirectory := AnsiString(GetTwainDirectory());
    {Continue only if twain is installed in an known directory}
    if TwainDirectory <> '' then
    begin

      fHandle := Windows.LoadLibraryA(PAnsiChar(TwainDirectory + TWAINLIBRARY));//ccc
      //fHandle := Windows.LoadLibraryA(PAnsiChar('TWAINDSM.dll'));
...
{Procedure to load or unloaded the twain source manager}
procedure TDelphiTwain.SetSelectedSourceIndex(const Value: Integer);
begin
...
function TDelphiTwain.SelectSource: Integer;
  ...
  begin
    ...
    xForm := TFormSelectSource.Create(nil);
    try
      for I := 0 to SourceCount-1 do
...
{Returns a source from the list}
function TDelphiTwain.GetSelectedSource: TTwainSource;
begin
  if SourceCount = 0 then begin
    Result := nil;
  end else begin
    if ((fSelectedSourceIndex < 0) and (SourceCount > 0)) or (fSelectedSourceIndex > SourceCount-1) then
      fSelectedSourceIndex := 0;

    Result := Source[fSelectedSourceIndex];
  end;
end;

function TDelphiTwain.GetSelectedSourceIndex: Integer;
begin
...
var
  ...
  xAppWndCallback: Pointer = nil;//WNDPROC = nil;
...
{Object being created}
constructor TDelphiTwain.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);

  {if not Assigned(aParentForm) or not aParentForm.Visible then
    raise Exception.Create('Error creating a TDelphiTwain object: aParentForm must be assigned and visible.');}
  ...
  if IsLibrary then begin
    ...
    if not Assigned(xAppWndCallback) then begin
      //xAppWndCallback := {%H-}{Windows.WNDPROC}(SetWindowLongPtr(WidgetSet.AppHandle,GWL_WNDPROC,{%H-}PtrInt(@AppWndCallback)));
      xAppWndCallback := {%H-}{Windows.WNDPROC}Pointer(SetWindowLongPtr(VirtualWindow,GWL_WNDPROC,{%H-}NativeInt(@AppWndCallback)));
    end;
  ...
  fInfo := TTwainIdentity.Create(nil);
...
begin
  ...
  begin
    ...
    if Owner.IsConsoleApplication then
      ...
      twUserInterface.hParent := GetActiveWindow;//GetForegroundWindow;
    ...
    //fEnabled := TRUE;
    Owner.fMessagesTimer.Enabled := TRUE;
    {Call method}
...
begin
  ...
  fEnabled := False;
  Owner.fMessagesTimer.Enabled := False;
end;
...
begin
  ...
  case ItemType of
    ...
    TWTY_STR64,
    TWTY_STR128,
    TWTY_STR255 :         Return := String(PAnsiChar(Data));//ccc
...
begin
  //Result := (Color and $FF) shl 16 + (Color and $00FF00) + (Color and $FF0000) shr 16;
  Result := (Color and $0000FF) + (Color and $00FF00) shl 8 + (Color and $FF0000) shr 8;
  Result := (Result and $FF) shl 16 + (Result and $00FF00) + (Result and $FF0000) shr 16;
end;
...
begin
  ...
  {If it was sucessful, return values}
  if Result = crSuccess then
  begin
    {Make list}
    for i := Low(List) to High(List) do
...
begin
  ...
  begin
    ...
    if (Container=TWON_ENUMERATION) or (Container=TWON_ARRAY) then
    {   }
    begin
    ...
    else if (Container=TWON_RANGE) then
     begin
      RangeV := GlobalLock(Handle);
      if RangeV^.ItemType <> TWTY_FIX32 then Result := crUnsupported
      else begin
        {Set array size}
        SetLength(Values, 2);
        {Fill array}
        Values[0] := RangeV^.MinValue;
        Values[1] := RangeV^.MaxValue;
        Return := RangeV.CurrentValue;
      end;
